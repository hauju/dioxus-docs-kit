...
<AccordionGroup>
  ...
  <Accordion title="How does syntax highlighting work?">
    Code blocks are lexed by `hl-lite`, a dependency-free highlighter built into the kit — twelve languages for about 48 KB of wasm, with nothing to enable. Each token becomes a `<span class="hl-{kind}">` and the colors come from the `--dk-hl-*` CSS tokens, so you restyle them in CSS. A fence whose language is unknown renders as plain text.
  </Accordion>
</AccordionGroup>
