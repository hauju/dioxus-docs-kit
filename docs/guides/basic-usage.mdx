...
<AccordionGroup>
  ...
  <Accordion title="How does syntax highlighting work?">
    Code blocks are highlighted by `dioxus-code`, which compiles tree-sitter grammars into the wasm. Each language sits behind a `lang-*` cargo feature so you only ship the grammars you fence; a block whose grammar is not compiled in renders as plain text.
  </Accordion>
</AccordionGroup>
