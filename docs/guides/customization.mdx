...
<Note>
Each page path must correspond to a `.mdx` file in the `docs/` directory. The `build.rs` script reads `_nav.json` and parses every listed page into the content bundle at compile time.
</Note>
...
```
  ...
  <Step title="Rebuild">
    Run `dx serve` — the build script re-generates the content bundle automatically.
  </Step>
</Steps>
...
The spec is parsed in `build.rs`, alongside your MDX pages:
...
- The second argument is the path to the OpenAPI YAML (or JSON) file
...
- Needs the `openapi` cargo feature on `dioxus-docs-kit` (on by default). Turning it off drops the endpoint pages and API sidebar; the parser itself lives in the build crate either way, so nothing extra ships to the browser
