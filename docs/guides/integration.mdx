...
<Steps>
  <Step title="Add dependencies">
    ...
    The kit's defaults bring `mermaid`, `highlight`, `openapi` and a common set of `lang-*` syntax-highlighting grammars. To ship less (for example only the grammars you actually fence), set `default-features = false` and list the features you need — see the crate README's *Syntax Highlighting* section.
  </Step>
  ...
  <Step title="Wire up the registry">
    ...
    To include an OpenAPI spec, chain `.with_openapi()` (needs the `openapi` feature, on by default):
