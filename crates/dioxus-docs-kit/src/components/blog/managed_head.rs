...
/// A head element owned by the current blog page.
#[derive(Clone, PartialEq)]
pub(super) struct HeadTag {
    tag: &'static str,
...
#[component]
pub(super) fn ManagedBlogHead(title: String, tags: Vec<HeadTag>) -> Element {
    ...
    use_effect(use_reactive!(|tags| {
        ...
        document::eval(&format!(
            r#"
            const generation = (window.__dkBlogHeadGeneration || 0) + 1;
            window.__dkBlogHeadGeneration = generation;
            window.__dkBlogHeadOwner = {owner};
            requestAnimationFrame(() => {{
                if (window.__dkBlogHeadGeneration !== generation) return;
                document.head.querySelectorAll('[data-dk-blog-head]').forEach(node => node.remove());
                for (const item of {payload}) {{
                    const node = document.createElement(item.tag);
                    for (const [key, value] of item.attributes) node.setAttribute(key, value);
                    node.setAttribute('data-dk-blog-head', '{owner}');
    ...
    use_drop(move || {
        document::eval(&format!(
            r#"
            if (window.__dkBlogHeadOwner === {owner}) {{
                window.__dkBlogHeadGeneration = (window.__dkBlogHeadGeneration || 0) + 1;
                document.head.querySelectorAll('[data-dk-blog-head]').forEach(node => node.remove());
            }}
        "#
        ));
        ...
        for (index, tag) in initial.iter().enumerate() {
            match tag.tag {
                ...
                _ => rsx! { document::Script {
                    key: "{index}", r#type: "application/ld+json", "data-dk-blog-head": "initial", "{tag.text}"
                } },
            }
        }
    }
